...
void enable_raw_mode() {
  ...
  raw.c_lflag &= ~(
      ECHO | ICANON); // c_lflag — это поле структуры
                      // представляющее собой битовую маску
                      // объединяет значения флагов ECHO и ICANON в одну битовую
                      // маску применяет маску ~(ECHO | ICANON) к текущему
                      // значению raw.c_lflag, оставляя все биты, кроме ECHO и
                      // ICANON, без изменений, а эти два бита сбрасывает в 0.
